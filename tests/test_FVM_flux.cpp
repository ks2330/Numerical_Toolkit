...
TEST(WallFluxTest, ZeroMassEnergy_MomentumIsPressureTimesNormal) {
    nt::fvm::GasModel gm{1.4, 287.0};
    nt::fvm::ConservativeState s = gm.toConservative({1.0, 2.0, 3.0, 2.0});
    nt::fvm::Vec2 n{0.6, 0.8};                                                

    nt::fvm::FluxVector f = wallFlux(s, n, gm);

    EXPECT_NEAR(f.mass,   0.0,       1e-12); 
    EXPECT_NEAR(f.energy, 0.0,       1e-12);
    EXPECT_NEAR(f.x_mom,  2.0 * 0.6, 1e-12);
    EXPECT_NEAR(f.y_mom,  2.0 * 0.8, 1e-12);
}


TEST(WallResidualTest, ClosedBodyUniformPressureZeroNetForce) {
    nt::fvm::GasModel gm{1.4, 287.0};
    const std::vector<meshgeneration::Node>    wallNodes = { EN(0,0,0), EN(4,0,1), EN(0,3,2) };
    const std::vector<meshgeneration::Element> wallElems = { E(0,1,2) };
    const auto wallFaces = nt::fvm::buildFaces(wallElems, wallNodes);

    nt::fvm::ConservativeState U = gm.toConservative({1.0, 0.0, 0.0, 1.0});
    std::vector<nt::fvm::ConservativeState> state(1, U);

    auto residual = computeResidual(state, wallFaces, gm);
...
TEST(FarFieldFluxTest, MatchingFreeStreamGivesPhysicalFlux) {
    nt::fvm::GasModel gm{1.4, 287.0};
    nt::fvm::ConservativeState Uinf = gm.toConservative({1.0, 0.5, 0.2, 1.5});
    nt::fvm::Vec2 n{0.6, 0.8};

    nt::fvm::FluxVector ff   = farFieldFlux(Uinf, Uinf, n, gm);
    nt::fvm::FluxVector phys = computeFlux(Uinf, n, gm);

    EXPECT_NEAR(ff.mass,   phys.mass,   1e-10);
    EXPECT_NEAR(ff.x_mom,  phys.x_mom,  1e-10);
    EXPECT_NEAR(ff.y_mom,  phys.y_mom,  1e-10);
    EXPECT_NEAR(ff.energy, phys.energy, 1e-10);
}

TEST(FarFieldResidualTest, FreeStreamPreservedThroughBoundary) {
    nt::fvm::GasModel gm{1.4, 287.0};

    const std::vector<meshgeneration::Edge> boundaryEdges = {
        {0,3,-1,9}, {1,3,-1,9}, {1,4,-1,9}, {2,4,-1,9}, {2,5,-1,9}, {0,5,-1,9}
    };
    const auto ffFaces = nt::fvm::buildFaces(elements, nodes, boundaryEdges);

    nt::fvm::ConservativeState Uinf = gm.toConservative({1.0, 0.5, 0.2, 1.5});
    std::vector<nt::fvm::ConservativeState> state(4, Uinf);

    auto residual = computeResidual(state, ffFaces, gm, Uinf, 9);

    for (size_t i = 0; i < residual.size(); ++i) {
        EXPECT_NEAR(residual[i].rho,   0.0, 1e-10) << "cell " << i;
        EXPECT_NEAR(residual[i].rho_u, 0.0, 1e-10) << "cell " << i;
        EXPECT_NEAR(residual[i].rho_v, 0.0, 1e-10) << "cell " << i;
        EXPECT_NEAR(residual[i].rho_e, 0.0, 1e-10) << "cell " << i;
    }
}


TEST(MixedBCResidualTest, FarFieldCellZero_WallCellBlocksMass) {
    nt::fvm::GasModel gm{1.4, 287.0};

    const std::vector<meshgeneration::Edge> boundaryEdges = {
        {0,3,-1,9}, {1,3,-1,9}
    };
    const auto mixFaces = nt::fvm::buildFaces(elements, nodes, boundaryEdges);

    nt::fvm::ConservativeState Uinf = gm.toConservative({1.0, 0.5, 0.2, 1.5});
    std::vector<nt::fvm::ConservativeState> state(4, Uinf);

    auto residual = computeResidual(state, mixFaces, gm, Uinf, 9);

    EXPECT_NEAR(residual[1].rho,   0.0, 1e-10);
    EXPECT_NEAR(residual[1].rho_u, 0.0, 1e-10);
    EXPECT_NEAR(residual[1].rho_v, 0.0, 1e-10);
    EXPECT_NEAR(residual[1].rho_e, 0.0, 1e-10);

    EXPECT_GT(std::abs(residual[2].rho), 1e-3);
}
