...
namespace nt::fvm
{
    ...
    inline FluxVector wallFlux(const ConservativeState& interior, const Vec2& normal, const GasModel& gm) {
        double p = gm.pressure(interior);
        return FluxVector{0.0, p * normal.x, p * normal.y, 0.0};
    }

    inline FluxVector farFieldFlux(const ConservativeState& interior, const ConservativeState& freeStream, const Vec2& normal, const GasModel& gm) {
        return RusanovFlux(interior, freeStream, normal, gm);
    }

    inline std::vector<ConservativeState> computeResidual(
            const std::vector<ConservativeState>& state,
            const std::vector<Face>& faces,
            const GasModel& gm,
            ConservativeState Uinf = {0.0, 0.0, 0.0, 0.0},
            int farFieldGroup = -2) {
        ...
        for (const auto& face : faces) {
            ...
            FluxVector flux;
            if (face.rightElement_id != -1) {                 // interior
                flux = RusanovFlux(sL, state[face.rightElement_id], face.normal, gm);
            } else if (face.bcType == farFieldGroup) {        
                flux = farFieldFlux(sL, Uinf, face.normal, gm);
            } else {                                          
                flux = wallFlux(sL, face.normal, gm);
            }
            ...
            res[face.leftElement_id] = res[face.leftElement_id] - contrib;        // outflow from left
            if (face.rightElement_id != -1)                                       // credit right only for interior
